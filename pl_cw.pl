...
%
% The cut operator '/' is intimately related with backtracking since it terminates it. This operator will always be true when conbined with in a rule clause but it will "break" the backtracking once it first happens. 
% Prolog in based in first logic order but it violates one of its principles, as in prolog language the order of predicates matters in contrast with first order logic. The cut operator is used then to take advantage of this in order to intentionally fail once the first condition was found.
%
%
%
%
